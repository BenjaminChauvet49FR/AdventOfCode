...
function conclusion_6_1() {
	...
	for (var i = 0 ; i < rawData.length ; i++) {
		if (rawData[i].length == 0) {
			answer += counted.length;
			counted = [];
		} else {
			for (var j = 0 ; j < rawData[i].length ; j++) {				
				if (counted.indexOf(rawData[i][j]) == -1) {
					counted.push(rawData[i][j]);
				}
			}
		}
	}
	answer += counted.length;
	return answer; // Correct answer = 6457
}
...
function conclusion_6_2() {
	...
	for (var i = 0 ; i < rawData.length ; i++) {
		if (rawData[i].length == 0) {
			...
			counted = counted.filter(function(charac) {return rawData[i].indexOf(charac) != -1});
		} else {
			counted = stringToCharArray(rawData[i]);
		}
	}
	answer += counted.length;
	return answer;
