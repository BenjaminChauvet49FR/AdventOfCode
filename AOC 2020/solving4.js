...
function conclusion_4_1() {
	...
	var countPres = 0;
	var string;
	var present = generateArray(expected_fields.length-1, true);
	var i, j;
	for (i = 0 ; i < rawData.length ; i++) {
		string = rawData[i];
		if (string == "") {			
			if (countPres == expected_fields.length-1) {
				countValid++;
			}
			countPres = 0;
		} else {
			for (j = 0 ; j < expected_fields.length-1 ; j++) {
				if (string.indexOf(expected_fields[j]) != -1) {					
					countPres++; // I hope no field is duplicated !
				}
			}
		}
	}
	return countValid; // Correct answer = 228 (out of 256)
...
function conclusion_4_2() {
	...
	var canBeValid = true;
	var string;
	var present = generateArray(expectedFields.length-1, true);
	var i, j;
	for (i = 0 ; i < rawData.length ; i++) {
		string = rawData[i];
		if (string == "") {			
			if (countPres == expectedFields.length-1) {
				countValid++;
			}
			countPres = 0;
			canBeValid = true;
		} else if (canBeValid) {
			for (j = 0 ; j < expectedFields.length-1 ; j++) {
				data = getData(string, expectedFields[j]);
				if (data != null) {					
					if (isValid(data, j)) {	
						console.log("Valid field : " + expectedFields[j] + ":" + data);
						countPres++; // I assume no field is duplicated !
					} else {
						console.log("Rejected field : " + expectedFields[j] + ":" + data);
						canBeValid = false;
						break;
					}
				}
...
function getData(p_string, p_field) {
	const pos = p_string.indexOf(p_field);
	if (pos == -1) {
		return null;
	}
	const posMin = pos+4;
	var posMax = posMin+1;
	while (posMax < p_string.length && p_string[posMax] != " ") {
		posMax++;
	}
	return p_string.substring(posMin, posMax); // That's how substring works.
}

function isValid(p_data, p_index) {
	switch (p_index) {
		case 0 :
			var val = parseInt(p_data, 10);
			return (val != NaN && val >= 1920 && val <= 2002);
		break;
		case 1 :
			var val = parseInt(p_data, 10);
			return (val != NaN && val >= 2010 && val <= 2020);
		break;
		case 2 :
			var val = parseInt(p_data, 10);
			return (val != NaN && val >= 2020 && val <= 2030);
		break;
		case 3 :
			var unit = p_data.substring(p_data.length-2);
			var val = parseInt(p_data.substring(0,p_data.length-2), 10);
			if (val == NaN) {
				return false;
			} else if (unit == "cm") {
		...
		break;
		case 4 :
			if (p_data.length != 7 || p_data[0] != "#") {
				return false;
			}
			for (var i = 1 ; i <= 6 ; i++) {
				if ((p_data[i] < 'a' || p_data[i] > 'f') && (p_data[i] < '0' || p_data[i] > '9')) {
					return false;
				}
			}
			return true;
		...
		case 5 :
			var found = false;
			["amb","blu","brn","gry","grn","hzl","oth"].forEach(colour => {
				found = found || (colour == p_data);
			});
			return found;
		break;
		case 6 : 
