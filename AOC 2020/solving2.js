...
function conclusion_2_1() {
	var tokens, tokens2, min, max, charac;
	var count = 0;
	var countLetter;
	for (var i = 0 ; i < rawData.length ; i++) {
		tokens = rawData[i].split(" ");
		tokens2 = tokens[0].split("-");
		min = parseInt(tokens2[0], 10);
		max = parseInt(tokens2[1], 10);
		charAc = tokens[1][0];
		countLetter = 0;
		for (var j = 0 ; j < tokens[2].length ; j++) {
			if (tokens[2][j] == charAc) {
				countLetter++;
				if (countLetter > max) {
					break;
				}
	...
	return count; // Correct answer = 580
}

function conclusion_2_2() {
	var tokens, tokens2, pos1, pos2, charac;
	var count = 0;
	for (var i = 0 ; i < rawData.length ; i++) {
		tokens = rawData[i].split(" ");
		tokens2 = tokens[0].split("-");
		pos1 = parseInt(tokens2[0], 10);
		pos2 = parseInt(tokens2[1], 10);
		charAc = tokens[1][0];
		if ((tokens[2][pos1-1] == charAc) != (tokens[2][pos2-1] == charAc)) {
			count++;
		}
	}
	return count; // Correct answer = 
}