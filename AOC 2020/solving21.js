...
function init() {
	gData.listIngredients = [];
	gData.listAllergens = [];
	gData.meals = [];
	gData.numberIngredients = 0;
	gData.numberAllergens = 0;
	gData.mealsPerIngredient = []; // Number of meals each ingredient appear in
	
	// Initialize all meals + number of ingredients and allergens.
	var index;
	var tokens, tokensIngredients, tokensAllergens;
	for (var i = 0 ; i < rawData.length ; i++) {
		gData.meals.push({ingredients : [], allergens : []});
		tokens = rawData[i].split(" (contains ");
		tokensIngredients = tokens[0].split(" ");
		for (var j = 0 ; j < tokensIngredients.length ; j++) {
			index = findOrCreateIndex(gData.listIngredients, tokensIngredients[j]);
			gData.meals[i].ingredients.push(index);
			if (gData.mealsPerIngredient.length != index) {
				gData.mealsPerIngredient[index]++;
			} else {
				gData.mealsPerIngredient.push(1); // New ingredient : appears in one meal so far !
			}
			
		}
		tokensAllergens = tokens[1].substring(0, tokens[1].length-1).split(", ");
		for (var j = 0 ; j < tokensAllergens.length ; j++) {
			index = findOrCreateIndex(gData.listAllergens, tokensAllergens[j]);
			gData.meals[i].allergens.push(index);
		}
	}
	gData.numberIngredients = gData.listIngredients.length;
	gData.numberAllergens = gData.listAllergens.length;
	
	// x : meal, y : ingredient
	const mealContainsIngredient = generateDoubleEntryArray(gData.meals.length, gData.numberIngredients, false);
	// x : meal, y : allergen
	const mealMentionsAllergen = generateDoubleEntryArray(gData.meals.length, gData.numberAllergens, false);
	// x : ingredient, y : potential allergen
	gData.ingredientMayContainAllergen = generateDoubleEntryArray(gData.numberIngredients, gData.numberAllergens, true);
	var iMeal;
	var meal;
	for (iMeal = 0 ; iMeal < gData.meals.length ; iMeal++) {
		meal = gData.meals[iMeal];
		meal.ingredients.forEach(iItem => {
			mealContainsIngredient[iItem][iMeal] = true;
		});
		meal.allergens.forEach(iItem => {
			mealMentionsAllergen[iItem][iMeal] = true;
		});
	}
		
		
	var iAll, iMeal, iIng;
	// For each allergen, for each meal that mentions this allergen, make at false all the ingredients not in this meal. 
	for (iAll = 0 ; iAll < gData.numberAllergens ; iAll++) {
		for (iMeal = 0 ; iMeal < gData.meals.length ; iMeal++) {
			if (mealMentionsAllergen[iAll][iMeal]) {
				for (iIng = 0 ; iIng < gData.numberIngredients ; iIng++) {
					if (!mealContainsIngredient[iIng][iMeal]) {
						gData.ingredientMayContainAllergen[iAll][iIng] = false;
					}
				}
			}
		}
...
function findOrCreateIndex(p_listNames, p_name) {
	for (var i = 0 ; i < p_listNames.length ; i++) {
		if (p_listNames[i] == p_name) {
			return i;
		}
	}
	p_listNames.push(p_name);
	return p_listNames.length-1;
}

function conclusion_21_1() {
	// An ingredient cannot contain an allergen IF said allergen is present on any meal that doesn't contain it. 
	// Create a big array of allergens x ingredients, initially at true.
	// For each allergen :
		// For each meal that contains this allergen :
			// Make at "false" the entries (this allergen) x (all ingredients that are NOT part of this meal).
	// Then, check the ingredients that have all their allergens at false. (conclusion requires to know in how many meals each ingredient appear)
	init();
	...
	for (iIng = 0 ; iIng < gData.numberIngredients ; iIng++) {
		iAll = 0;
		while (iAll < gData.numberAllergens && !gData.ingredientMayContainAllergen[iAll][iIng]) {
			iAll++;
		}
		if (iAll == gData.numberAllergens) {
			answer += gData.mealsPerIngredient[iIng];
		}
	}
	return answer;
} // Correct answer = 2573. Hey, I treated this part quite well.


var gData2 = {
	safeIngredientArray : [],
	dangerousIngredientsList : [] // Only 8 of them
};

function declareSafeIngredients() {
	var iAll, iIng;
	gData2.safeIngredientArray = generateArray(gData.meals.length, false);
	gData2.dangerousIngredientsList = [];
	for (iIng = 0 ; iIng < gData.numberIngredients ; iIng++) {
		iAll = 0;
		while (iAll < gData.numberAllergens && !gData.ingredientMayContainAllergen[iAll][iIng]) {
			iAll++;
		...
		if (iAll == gData.numberAllergens) {
			gData2.safeIngredientArray[iIng] = true;
		} else {
			gData2.dangerousIngredientsList.push(iIng);
		}
	}
}
...
function createCriticalList() {
	...
	for (iMeal = 0 ; iMeal < gData.meals.length ; iMeal++) {
		...
		meal = gData.meals[iMeal];
		for (iIIM = 0 ; iIIM < meal.ingredients.length ; iIIM++) {
			if (!gData2.safeIngredientArray[meal.ingredients[iIIM]]) {
				dangerousIngredients.push(meal.ingredients[iIIM]);
				/*if (dangerousIngredients.length == meal.allergens.length) {
					break;
				}*/
			}
		}
		if (dangerousIngredients.length == meal.allergens.length) {
			gData2.mealsForAnalysis.push({idMeal : iMeal, dangerousIngredients : dangerousIngredients.slice(), allergens : meal.allergens});
		} // None ? The "mealsForAnalysis" list is always empty ? This was supposed to be the "critical list"...
		meal.dangerousIngredients = dangerousIngredients.sort(function(a, b) {return a-b}).slice();
		meal.allergens.sort(function(a,b){return(a-b)});
	}
}
...
{allergen: 2, ingredient: 40}
{allergen: 5, ingredient: 46}
{allergen: 7, ingredient: 50}
{allergen: 1, ingredient: 75}
{allergen: 3, ingredient: 59}
{allergen: 6, ingredient: 9}
{allergen: 4, ingredient: 83} 
{allergen: 0, ingredient: 44}
So in every meal where allergen 2 is mentioned, we MUST find ingredient 40, and so on...
	
Oh btw almost every meal contains 7 dangerous ingredients.	
	*/
	
	victoryLap();
	return gData; // read gDaa.stringOfIngredients and you'll find that correct answer = bjpkhx,nsnqf,snhph,zmfqpn,qrbnjtj,dbhfd,thn,sthnsg
}

function analyzeData() {
	gData.listCorrespondings = [];
	var dangerousIngredients = gData2.dangerousIngredientsList.slice();
	var nbPasses = 0;
	var newDangerousIngredients;
	var allergenThisIng;
	var ing, alle;
	while (dangerousIngredients.length > 0 && nbPasses < 8) {
		newDangerousIngredients = [];
		for (var i = 0 ; i < dangerousIngredients.length ; i++) {
			ing = dangerousIngredients[i]; // Please, make such that only ONE allergen matches this ingredient.
			allergenThisIng = -1;
			for (alle = 0 ; alle < gData.numberAllergens ; alle++) {
				if (gData.ingredientMayContainAllergen[alle][ing]) {					
					if (allergenThisIng != -1) {
						allergenThisIng = -1;
						break;
					} else {
						allergenThisIng = alle;
					}
				}
			} if (allergenThisIng == -1) {
				newDangerousIngredients.push(ing); // not found an allergen matching an ingredient...
			} else { // Yeah, found one !
				var all2, ing2;
				for (var all2 = 0 ; all2 < gData.numberAllergens ; all2++) {
					gData.ingredientMayContainAllergen[all2][ing] = false;					
				}
				for (var i2 = 0 ; i2 < gData2.dangerousIngredientsList.length ; i2++) {
					ing2 = gData2.dangerousIngredientsList[i2];
					gData.ingredientMayContainAllergen[allergenThisIng][ing2] = false;
				}
				gData.ingredientMayContainAllergen[allergenThisIng][ing] = true; // Useless now, but symbolical.
				gData.listCorrespondings.push({allergen : allergenThisIng, ingredient : ing});
			}
		}
		dangerousIngredients = newDangerousIngredients;
		nbPasses++;
	}
}

function victoryLap() {
	sortAlphabetical(gData.listCorrespondings, function(p_corresp) {return gData.listAllergens[p_corresp.allergen]});
	gData.stringOfAllergens = gData.listAllergens[gData.listCorrespondings[0].allergen];
	gData.stringOfIngredients = gData.listIngredients[gData.listCorrespondings[0].ingredient];
	for (var i = 1 ; i < gData.listCorrespondings.length ; i++) {
		gData.stringOfAllergens += ","+gData.listAllergens[gData.listCorrespondings[i].allergen];
		gData.stringOfIngredients += ","+gData.listIngredients[gData.listCorrespondings[i].ingredient];
	}	
}