...
function init() {
	...
	for (i = 0 ; i < rawData.length ; i++) {
		containing = [];
		amounts = [];
		tokens = rawData[i].split(" bag");
		indexRule = indexFindOrPut(tokens[0]);
		if (tokens[tokens.length-2].indexOf("no other") == -1) {	
			for (j = tokens.length-2 ; j >= 1 ; j--) { // Last token = "s." or "."
				tokens2 = tokens[j].split(" ");
				containing.push( indexFindOrPut(tokens2[tokens2.length-2]+" "+tokens2[tokens2.length-1]) );
				amounts.push(parseInt(tokens2[tokens2.length-3]));
			}
		}	
		gRules.push({
			container : indexRule,
			contains : containing.slice(),
			containedBy : [],
			amounts : amounts.slice(),
			numberOfBags : -1, // For part 2
		});
	}
	sortAscending(gRules, [function(r) {return r.container}]);
	
	for (var i = 0 ; i < gRules.length ; i++) {
		gRules[i].contains.forEach(ind => {
			gRules[ind].containedBy.push(i);
		});
	}
}

function indexFindOrPut(p_bag) {
	var index = gBags.indexOf(p_bag);
	if (index != -1) {
		return index;
	}
	gBags.push(p_bag);
	return gBags.length-1;
}

function conclusion_7_1() {
	init();
	
	var indexSGB = indexFindOrPut("shiny gold");
	var listContainsSGB = gRules[indexSGB].containedBy.slice();
	var amount = 0;
	var newList, rule;
	while(listContainsSGB.length > 0) {
		newList = [];
		listContainsSGB.forEach(index => {
			rule = gRules[index];
			if (!rule.alreadyCounted) {
				amount++;
				rule.alreadyCounted = true;
				rule.containedBy.forEach(id => {
					newList.push(id);
				});
			}			
		});
		listContainsSGB = newList;
	}
	return amount;
}
...
function mustContain(p_index) {
	if (gRules[p_index].numberOfBags != -1) {
		return gRules[p_index].numberOfBags;
	}
	var answer = 0;
	for (var i = 0 ; i < gRules[p_index].contains.length ; i++) {
		answer += (mustContain(gRules[p_index].contains[i]) + 1) * gRules[p_index].amounts[i];
	}
	gRules.numberOfBags = answer;
	return answer;
}
