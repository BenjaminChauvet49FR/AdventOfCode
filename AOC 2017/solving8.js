const rawData=`uz inc 134 if hx > -10
qin dec -300 if h <= 1
...
he inc -446 if xp > -2075`.split("\n");

var gRegisters, gInstructions;
...
function init() {
	...
	rawData.forEach(str => {
		...
		gInstructions.push({
			registerM : getIndexInListWithAdd(listTokens, tokens[0]),
			modificator : (parseInt(tokens[2], 10) * (tokens[1] == "inc" ? 1 : -1)),
			registerC : getIndexInListWithAdd(listTokens, tokens[4]),
			comparator : comp,
			compared : parseInt(tokens[6], 10)
		})
	});
	gRegisters = generateArray(listTokens.length, 0);
}

function executeInstruction(p_inst) {
	var val = gRegisters[p_inst.registerC];
	var doIt;
	switch (p_inst.comparator) {
		case ELR : doIt = val <= p_inst.compared; break;
		case EHR : doIt = val >= p_inst.compared; break;
		case HR : doIt = val > p_inst.compared; break;
		case LR : doIt = val < p_inst.compared; break;
		case NE : doIt = val != p_inst.compared; break;
		case E : doIt = val == p_inst.compared; break;
	}
	if (doIt) {
		gRegisters[p_inst.registerM] += p_inst.modificator;
	}
}

function conclusion_8_1() {
	init();
	gInstructions.forEach(instruction => {
		executeInstruction(instruction);
	});
	var answer = gRegisters[0];
	for (var k = 1 ; k < gRegisters.length ; k++) {
		answer = Math.max(answer, gRegisters[k]);
	}
	return answer;
}
...
function conclusion_8_2() {
	...
	gInstructions.forEach(instruction => {
		executeInstruction(instruction);
		val = gRegisters[instruction.registerM];
		maxEver = Math.max(maxEver, val);
	});
	return maxEver;
}