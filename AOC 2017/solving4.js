const rawData=`bdwdjjo avricm cjbmj ran lmfsom ivsof
...
function isValidP2(p_str) {
	var tokens = p_str.split(" ");
	for (var j = 1 ; j < tokens.length ; j++) {
		for (var k = 0 ; k < j ; k++) {
			if (areAnagrams(tokens[j], tokens[k])) {
				return false;
			}
		}
	}
...
function areAnagrams(p_str1, p_str2) {
	if (p_str1.length != p_str2.length) {
		return false;
	}
	var occurences1 = generateArray(26, 0);
	var occurences2 = generateArray(26, 0);
	for (var i = 0 ; i < p_str1.length ; i++) {
		occurences1[charToASCIIRelativePosition(p_str1.charAt(i),'a')]++;
		occurences2[charToASCIIRelativePosition(p_str2.charAt(i),'a')]++;
	}
	var j;
	for (j = 0 ; j < 26 ; j++) {
		if (occurences1[j] != occurences2[j]) {
			return false;
		}
	}
	return true;
