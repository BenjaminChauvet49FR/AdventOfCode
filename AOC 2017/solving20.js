const rawData=`p=<1500,413,-535>, v=<-119,22,36>, a=<-5,-12,3>
...
function init() {
	...
	var tokens;
	rawData.forEach(str => {
		tokens = str.split(/<|>/); // Regex split
		gData.push({p : arrayFromString(tokens[1]), v : arrayFromString(tokens[3]), a : arrayFromString(tokens[5])});
	});
}
...
/*function conclusion_20_1() {
	...
	for (time = 0 ; time < 10000 ; time++) {
		for (part = 0 ; part < gData.length ; part++) {
			for (axis = 0 ; axis <= 2; axis++) {
				gData[part].v[axis] += gData[part].a[axis];
				gData[part].p[axis] += gData[part].v[axis];
			}
		}
	}
	var val;
	...
	for (part = 0 ; part < gData.length ; part++) {
		val = 0;
		for (axis = 0 ; axis <= 2 ; axis++) {
			val += Math.abs(gData[part].p[axis]);
		}
		if (min == -1 || val < min) {
			bestPart = part;
			min = val;
...
// Which one is gonna stay closer to point 0 in the long run ?
// Well, first, it plays between all of those with the smallest acceleration (in absolute values). 
// Followed by those with the smallest velocity.
function conclusion_20_1() { 
	init();
	var part, axis, movingAway;
	var minAcc = 0;
	for (axis = 0 ; axis <= 2; axis++) {
		minAcc += Math.abs(gData[0].a[axis]);
	}
	var bestParts = [0];
	var acc;
	for (var part = 0 ; part < gData.length ; part++) {
		acc = 0;
		for (axis = 0 ; axis <= 2; axis++) {
			acc += Math.abs(gData[part].a[axis]);
		}
		if (acc < minAcc) {
			minAcc = acc;
			bestParts = [part];
	...
	// Is there only one best part ? If not, we have to make them run for a while...
	if (bestParts.length == 1) {
		return bestParts[0];
	} else {
		var part, validateMe;
		var areParticlesValidated = generateArray(false, bestParts.length);
		var particlesToValidate = bestParts.length;
		while (particlesToValidate > 0) {			
			for (var i = 0 ; i < bestParts.length ; i++) {
				part = bestParts[i];
				validateMe = true;
				for (axis = 0 ; axis <= 2; axis++) {
					gData[part].v[axis] += gData[part].a[axis];
					gData[part].p[axis] += gData[part].v[axis];
					if (!sameSign(gData[part].v[axis], gData[part].a[axis]) || !sameSign(gData[part].v[axis], gData[part].p[axis])) {
						validateMe = false;
					}
				}
				if (validateMe && !areParticlesValidated[i]) {
					areParticlesValidated[i] = true;
					particlesToValidate--;
				}
			}
		}
		// Now, all particles are validated (e.g. their positions, accelerations and speed all have the same sign) : let's find the closest one.
		var bestPart = bestParts[0];
		var minPos = 0;
		for (axis = 0 ; axis <= 2; axis++) {
			minPos += Math.abs(gData[bestPart].p[axis]);
		}
		var pos;
		for (var i = 1 ; i < bestParts.length ; i++) {
			...
			part = bestParts[i];
			for (axis = 0 ; axis <= 2; axis++) {
				pos += Math.abs(gData[part].p[axis]);
			}
			if (minPos > pos) {
				minPos = pos;
				bestPart = part;
...
function sameSign(p_val1, p_val2) {
	return (p_val1 == 0 || p_val2 == 0 || (p_val1 > 0 == p_val2 > 0)); 
}

// ------------------------

var gStillThere, gNumberStillThere;

// The indexes are the ones of the particles still in place, not sorted.
function eliminateCollisions(p_indexes) {
	sortAscending(p_indexes, [
		function(x) {return gData[x].p[0]},
		function(x) {return gData[x].p[1]},
		function(x) {return gData[x].p[2]}]);
	var indexesLeft = 0;
	var newIndexes = [];
	var i = 0;
	var part, nextPart, prevPart, equality;
	
	while (i < p_indexes.length) {
		part = p_indexes[i];
		equality = false;
		if (i < p_indexes.length-1) {
			nextPart = p_indexes[i+1];
			equality = gData[part].p[0] == gData[nextPart].p[0] && gData[part].p[1] == gData[nextPart].p[1] && gData[part].p[2] == gData[nextPart].p[2];
		}
		if (!equality) {
			if (i > 0) {				
				prevPart = p_indexes[i-1];
				equality = gData[part].p[0] == gData[prevPart].p[0] && gData[part].p[1] == gData[prevPart].p[1] && gData[part].p[2] == gData[prevPart].p[2];
			}
		} 
		if (equality) {
			if (gStillThere[part]) {
				gNumberStillThere--;
				gStillThere[part] = false;
			}
		} else {
			newIndexes.push(part);
		}
		i++;
	}
	return newIndexes;
...
function makePartsAdvance(p_particles) {
	var axis;
	p_particles.forEach(part => {
		for (axis = 0 ; axis <= 2; axis++) {
			gData[part].v[axis] += gData[part].a[axis];
			gData[part].p[axis] += gData[part].v[axis];
		}
...
function conclusion_20_2() {
	init();
	gStillThere = generateArray(gData.length, true);
	gNumberStillThere = gData.length;
	var particles = generateNumericArray(0, gData.length-1);
	var part;
	// Advance the parts for some time, then see if in each axis (x, y, z) the ascending order of the velocities match the ascending order of the acceleration AND the sign of the positions match the sign of the velocities.
	// If yes for both conditions, there will be no more collisions.
	var found = false;
	while (!found) {		
		for (var gen = 0 ; gen < 100 ; gen++) {
			particles = eliminateCollisions(particles);
			makePartsAdvance(particles);		
		}
		var axis, nextPart, j;
		for (axis = 0 ; axis <= 2 ; axis++) {			
			sortAscending(particles, [function(x) {return gData[x].v[axis]}]);
			for (j = 0 ; j < particles.length-1 ; j++) {
				part = particles[j];
				nextPart = particles[j+1];
				if (gData[part].a[axis] > gData[nextPart].a[axis] || 
				!(sameSign(gData[part].v[axis], gData[part].p[axis]) && sameSign(gData[part].v[axis], gData[part].a[axis])) ) {
					break;
				}
			}
			...
			if (!(sameSign(gData[nextPart].v[axis], gData[nextPart].p[axis]) && sameSign(gData[nextPart].v[axis], gData[nextPart].a[axis])) ) {
				break;
			}
		}
		found = axis == 3;
	}
	return gNumberStillThere;
}
