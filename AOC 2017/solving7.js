...
var gTokens;
function init() {
	gData = generateArrangedArray(rawData.length, function(x) {return {}});
	gTokens = [];
	var tokens, tokens2, index;
	
	// Ascending part
	rawData.forEach(str => {
		tokens = str.split(") -> ");
		tokens2 = tokens[0].split(" (");
		index = getIndexInListWithAdd(gTokens, tokens2[0]);
		gData[index].weight = parseInt(tokens2[1], 10);
		gData[index].supports = [];
		gData[index].isSupportedBy = -1;
		if (tokens.length > 1) {			
			tokens2 = tokens[1].split(", ");
			tokens2.forEach(str2 => {
				gData[index].supports.push(getIndexInListWithAdd(gTokens, str2) );
			});
		}
	});
	...
	for (var i = 0 ; i < gData.length ; i++) {
		for (var j = 0 ; j < gData[i].supports.length ; j++) {
			gData[gData[i].supports[j]].isSupportedBy = i;
		}
	}
}
...
function conclusion_7_1() {
	...
	while (root != -1) {
		pr = root;
		root = gData[root].isSupportedBy;
	}
	return gTokens[pr];
}

function calculateWeights(p_root) {
	gData[p_root].totalWeight = gData[p_root].weight;
	gData[p_root].supports.forEach(index => {
		calculateWeights(index);
		gData[p_root].totalWeight += gData[index].totalWeight;
	});
}
...
function conclusion_7_2() {
	...
	// List all the nodes until we find one that has 3+ nodes, one of which is unbalanced, and that node only supports balanced things
	var wwData;
	for (var k = 0 ; k < gData.length ; k++) {
		wwData = wrongBalance(k);
		if (wwData != null && isBalanced(wwData.wrongIndex)) {
			return gData[wwData.wrongIndex].weight + (wwData.correctTotalWeight - gData[wwData.wrongIndex].totalWeight);
		}
	}	
}

// Returns the following data about p_index if it supports at least 2 programs :
// 1) the index (in gData) of the program with the wrong weight
// 2) the difference (positive or negative) of the correct totalWeight - the actual total weight of gData
function wrongBalance(p_index) {
	if (gData[p_index].supports.length > 2) {
		var i;
		for (i = 0 ; i < gData[p_index].supports.length-1 ; i++) {
			if (gData[gData[p_index].supports[i+1]].totalWeight 
		    	!= gData[gData[p_index].supports[i]].totalWeight) {
				break;
			}
		}
		if (i == gData[p_index].supports.length-1) {
			return null;
		}
		// Here, i is such that i and i+1 have different weights !
		const k = (i+2)%gData[p_index].supports.length;
		const isFirstIncorrect = gData[gData[p_index].supports[k]].totalWeight != 
								gData[gData[p_index].supports[i]].totalWeight;
		const correctIndex = gData[p_index].supports[i + (isFirstIncorrect ? 1 : 0)];
		const wrongIndex = gData[p_index].supports[i + (isFirstIncorrect ? 0 : 1)];
		const correctTotalWeight = gData[correctIndex].totalWeight;
		return {correctTotalWeight : correctTotalWeight, wrongIndex : wrongIndex}
	} 
	return null;
}

function isBalanced(p_index) {
	for (var k = 0 ; k < gData[p_index].supports.length-1 ; k++) {
		if (gData[gData[p_index].supports[k]].totalWeight != gData[gData[p_index].supports[k+1]].totalWeight) {
			return false;
		}
	}
	return true;
