const rawData=`bkwzkqsxq-tovvilokx-nozvyiwoxd-172[fstek]
...
hplazytkpo-prr-cpnptgtyr-379[prtya]`.split("\n");

function getValue(p_str) {
	var check = p_str.substring(p_str.length-6, p_str.length-1);
	var tokens = p_str.substring(0, p_str.length-7).split("-");
	var listChars = generateArrangedArray(26, function(x) {return {key : x, occurences : 0}});
	var k;
	for (var i = 0 ; i < tokens.length-1 ; i++) {
		for (var j = 0 ; j < tokens[i].length ; j++) {
			k = charToASCIIRelativePosition(tokens[i].charAt(j), 'a');
			listChars[k].occurences++;
		}
	}
	sortAscending(listChars, [function(e) {return -e.occurences}, function(e) {return e.key}] );
	for (var i = 0 ; i < 5 ; i++) {
		k = asciiRelativePositionToChar(listChars[i].key, 'a');
		if (k != check.charAt(i)) {
			return 0;
		}
	}
...
function conclusion_4_2() {
	...
	for (var i = 0 ; i < rawData.length ; i++) {
		val = getValue(rawData[i]);
		if (val > 0) {
			console.log(val + " " + rotateName(rawData[i], val));  // Copy the output to an editor to read it !
		}
	}
}

function rotateName(p_str, p_val) {
	const val = p_val%26;
	var str = p_str;
	var newStr;
	var c;
	for (var i = 0 ; i < val ; i++) {
		...
		for (var j = 0 ; j < str.length ; j++) {
			...
			if (c == 'z') {
				...
				newStr += asciiRelativePositionToChar(charToASCIIRelativePosition(c, 'a')+1, 'a');
			} else {
				newStr += " ";
			}
		}
