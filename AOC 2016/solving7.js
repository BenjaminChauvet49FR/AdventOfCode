const rawData=`xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
...
spwwppgjgfexuezrixp[rotgzyxzqxyrroafx]tkwxfiamzdjdqpftvq`.split("\n");

function isValid(p_string) {
	var found = false;
	var inside = 0;
	for (var i = 0 ; i <= p_string.length-4 ; i++) {
		if (hereABBA(p_string, i)) {
			if (inside > 0) {
				return false;
			} else {
				found = true;
...
function hereABBA(p_string, p_firstIndex) {
	const k = p_firstIndex;
	return (p_string.charAt(k) == p_string.charAt(k+3)) && (p_string.charAt(k+1) == p_string.charAt(k+2)) && (p_string.charAt(k) != p_string.charAt(k+1));
}
...
function hereABA(p_string, p_i) {
	return (p_string.charAt(p_i) == p_string.charAt(p_i+2) && p_string.charAt(p_i) != p_string.charAt(p_i+1)) ? 
		[p_string.charAt(p_i), p_string.charAt(p_i+1)] : null;
}

function hereMatchingBAB(p_string, p_j, p_aba, p_inside, p_inside2) {
	const bab = hereABA(p_string, p_j);
	return (bab != null && p_aba[0] == bab[1] && p_aba[1] == bab[0] && ((p_inside != 0) != (p_inside2 != 0)) );
}

function isValidP2(p_string) {
	var found = false;
	var inside = 0;
	var inside2;
	var aba;
	for (var i = 0 ; i <= p_string.length-3 ; i++) {
		aba = hereABA(p_string, i);
		if (aba != null) {
			inside2 = inside;
			for (var j = i+1 ; j <= p_string.length-3 ; j++) {
				if (p_string.charAt(j) == '[') {inside2++;}
				if (p_string.charAt(j) == ']') {inside2--;}
				if (hereMatchingBAB(p_string, j, aba, inside, inside2)) {
					return true;
				}
			}
		}
