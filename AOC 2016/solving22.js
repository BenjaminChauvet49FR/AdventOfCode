const rawData=`root@ebhq-gridcenter# df -h
...
/dev/grid/node-x38-y24   87T   73T    14T   83%`.split("\n");

function conclusion_22_1() {
	var answer = 0;
	for (var i = 2 ; i < rawData.length ; i++) {
		for (var j = i+1 ; j < rawData.length ; j++) {
			if (viableNode(rawData[i], rawData[j])) {
				answer++;
			}
			if (viableNode(rawData[j], rawData[i])) {
				answer++; // ERROR : I thought I had done it, but no !
			}
		}
	}
	return answer;
} // Not 966 (my bad), not 340 (too low ?)

function viableNode(p_str1, p_str2) {
	var tokens = p_str1.split(" ");
	var index = 0;
	var i = 0;
	var used1, avail2;
	while(true) {
		if (tokens[i].length != 0) {
			if (index == 2) {
				used1 = parseInt(tokens[i].substring(0, tokens[i].length-1), 10);				
				break;
			} else {
				index++;				
			}
	...
	if (used1 == 0) {
		return false; // ERROR : forgot this ! 
	}
	i = 0;
	tokens = p_str2.split(" ");
	index = 0;
	while(true) {
		if (tokens[i].length != 0) {
			if (index == 3) {
				avail2 = parseInt(tokens[i].substring(0, tokens[i].length-1), 10);				
				break;
			} else {
				index++;				
			}
...
function init() {
	...
	for (var k = 2 ; k < rawData.length ; k++) {
		...
		while(true) {
			if (tokens[i].length != 0) {
				if (index == 2) {
					used = parseInt(tokens[i].substring(0, tokens[i].length-1), 10);				
					index++;
				} else if (index == 3) {
					avail = parseInt(tokens[i].substring(0, tokens[i].length-1), 10);				
					break;
				} else {
					index++;				
				}
