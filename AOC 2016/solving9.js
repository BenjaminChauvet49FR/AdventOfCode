	...
	while (i < p_string.length) {
		...
		switch(c) {
			...
			case ')' : 
				...
				i++;
				for (j = 0 ; j < lRep ; j++) {
					caughtStr += p_string.charAt(i);
					i++;
				}
				if (p_isPart2) {					
					answer += tRep*countDecompressedLength(caughtStr, true); // I guess it will be p2, right ?
				} else {					
					answer += tRep * lRep;
				}
			break;
...
function conclusion_9_1() {
	return countDecompressedLength(rawData, false);
}

function conclusion_9_2() {
	return countDecompressedLength(rawData, true);
}
// I tried to solve part 2 before part 1 :)

/*// Straightforward ; Way too long;
function getOneDecompStep(p_string) {
	var answer = "";
	var i = 0;
	var c;
	var tRep; // Number of times to repeat ; after the x
