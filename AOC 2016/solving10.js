...
bot 23 gives low to bot 183 and high to bot 140`.split("\n");

function conclusion_10_1() {
	return conclusion_10_aux(false);
}

function conclusion_10_2() {
	return conclusion_10_aux(true);
}

function conclusion_10_aux(p_isPart2) {
	var starts = [];
	var bots = [];
	var tokens, n1, n2, n3, v2, v3;
	rawData.forEach(str => {
		tokens = str.split(" ");
		if (str.startsWith("v")) {
			n1 = parseInt(tokens[1], 10);
			n2 = parseInt(tokens[5], 10);
			starts.push({value : n1, dest : n2});
		} else {
			n1 = parseInt(tokens[1], 10);
			if (p_isPart2) {				
				n2 = parseInt(tokens[6], 10);
				v2 = (tokens[5] == "output");
				n3 = parseInt(tokens[11], 10);
				v3 = (tokens[10] == "output");
				bots.push({id : n1, destLow : n2, destHigh : n3, got : [], lowIsOutput : v2, highIsOutput : v3});								
			} else {
				n2 = (tokens[5] == "output") ? (-1) : parseInt(tokens[6], 10);
				n3 = (tokens[10] == "output") ? (-1) : parseInt(tokens[11], 10);
				bots.push({id : n1, destLow : n2, destHigh : n3, got : []});				
			}
		}
	});
	bots.sort(function(x,y) {return x.id-y.id});
	for (var i = 0 ; i < bots.length ; i++) {
		if (bots[i].id != i) {
			console.log("TODO : make an algorithm to find the bot in the list !");
		}
	}
	var iHave2Chips = [];
	var d;
	for (var i = 0 ; i < starts.length ; i++) {
		d = starts[i].dest;
		bots[d].got.push(starts[i].value);
		if (bots[d].got.length == 2) {
			iHave2Chips.push(d);
		}
	}
	var new2Chips;
	var bot, valMin, valMax, id1, id2;
	var answerP2 = 1;	
	var outputsNotPut = 3; // For P2
	while (iHave2Chips.length > 0) {
		new2Chips = [];
		for (var i = 0 ; i < iHave2Chips.length ; i++) {
			bot = bots[iHave2Chips[i]];
			valMin = Math.min(bot.got[0], bot.got[1]);
			valMax = Math.max(bot.got[0], bot.got[1]);			
			if (!p_isPart2) {
				if (valMin == 17 && valMax == 61) {
					return bot.id;
				}				
				[{d : bot.destLow, v : valMin}, {d : bot.destHigh, v : valMax}].forEach(elt=>{
					if (elt.d != -1) {
						bots[elt.d].got.push(elt.v);
						if (bots[elt.d].got.length == 2) {
							iHave2Chips.push(elt.d);
						}
					}
				});
			} else {
				[{d : bot.destLow, v : valMin, o : bot.lowIsOutput}, {d : bot.destHigh, v : valMax, o : bot.highIsOutput}].forEach(elt=>{
					if (!elt.o) {
						bots[elt.d].got.push(elt.v);
						if (bots[elt.d].got.length == 2) {
							iHave2Chips.push(elt.d);
						...
						answerP2 *= elt.v; // Only once for each output (0, 1, 2)
						outputsNotPut--;
					}
				});
				if (outputsNotPut == 0) {
