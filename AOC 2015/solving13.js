...
function init() {
	...
	var i1, i2, coef, val;
	rawData.forEach(str => {
		tokens = str.split(".")[0].split(" ");
		i1 = getIndexInListWithAdd(people, tokens[0]);
		i2 = getIndexInListWithAdd(people, tokens[10]);
		val = parseInt(tokens[3], 10);
		coef = (tokens[2].startsWith('g') ? 1 : -1);
		gData.push({
			p1 : i1,
			p2 : i2,
			hap : coef*val
	...
	gPeopleNumber = people.length;	
}

function conclusion_13_1() {
	init();
	gMatchUps = generateDoubleEntryArray(gPeopleNumber,gPeopleNumber, 0); 
	var i1, i2;
	for (var i = 0 ; i < gData.length ; i++) {
		i1 = gData[i].p1;
		i2 = gData[i].p2;
		gMatchUps[i1][i2] += gData[i].hap;
		gMatchUps[i2][i1] += gData[i].hap;
	}
	gIsTaken = generateArray(gPeopleNumber, false);
	gIsTaken[0] = true;
	gMax = 0;
	runBestPermutation(0, gPeopleNumber-1, [0], 0);
	return gMax;
}

// How to run all permutations !
function runBestPermutation(p_lastPerson, p_remain, p_array, p_sum) {
	if (p_remain == 0) {
		var total = gMatchUps[0][p_lastPerson]+p_sum;
		if (total > gMax) {
			gMax = total;
			gBestArray = p_array.slice();
		}
	} else {
		for (var i = 0 ; i < gIsTaken.length ; i++) {
			if (!gIsTaken[i]) {
				gIsTaken[i] = true;
				p_array.push(i);
				runBestPermutation(i, p_remain-1, p_array, p_sum + gMatchUps[p_lastPerson][i]);
				p_array.pop();
				gIsTaken[i] = false;
			}
		}
