...
var data = [];
const TOGGLE = 1, ON = 2, OFF = 3;


// Copy-pasted on 2018-3
function conclusion_6_1() {
	transformData();
	const indexes = makeArrayIndexes();
	var x, y, i, xxFind, yyFind;
	var total = 0;
	var slices;
	var gridBelongsToSlices;
	var slX, slY;
	var xx, yy, x1Find, x2Find, x2Find;
	var xMin, xMax, yMin, yMax;
	for (x = 0 ; x < 20 ; x++) {
		for (y = 0 ; y < 20 ; y++) {
			slices = makeSlices(indexes[x][y], x*50, x*50+49, y*50, y*50+49);
			slX = slices.xPart.length-1;
			slY = slices.yPart.length-1;
			gridLightedSlices = generateDoubleEntryArray(slY, slX, false); // Don't forget, it's dim2length, dim1length
			for (i = 0 ; i < indexes[x][y].length ; i++) {
				x1Find = 0;
				x2Find = 0;
				y1Find = 0;
				y2Find = 0;
				xMin = data[indexes[x][y][i]].x1; 
				xMax = data[indexes[x][y][i]].x2; 
				yMin = data[indexes[x][y][i]].y1; 
				yMax = data[indexes[x][y][i]].y2; 
				while(xMin > slices.xPart[x1Find]) { // [Slices in X are 200, 213, 241, 250]. Looking for xMin=213 : 1 ; 186 : 0
					x1Find++;
				}
				while(x2Find < slices.xPart.length-2 && xMax > slices.xPart[x2Find+1]-1) { // [Slices in X are 200, 213, 241, 250]. Looking for xMax=240 : 1 ; xMax = 294 : 2
					x2Find++;
				}			
				// We must know if x1 and x2 are in slice starting at 200, 213 or 241 in the example above. (xMax above 250 means the third slice)
				while(yMin > slices.yPart[y1Find]) { 
					y1Find++;
				}
				while(y2Find <= slices.yPart.length-2 && yMax > slices.yPart[y2Find+1]-1) { 
					y2Find++;
				}
				for (xx = x1Find ; xx <= x2Find ; xx++) {
					for (yy = y1Find ; yy <= y2Find ; yy++) {
						switch(data[indexes[x][y][i]].inst) {
							case TOGGLE : gridLightedSlices[xx][yy] = !gridLightedSlices[xx][yy]; break;
							case ON : gridLightedSlices[xx][yy] = true; break;
							case OFF : gridLightedSlices[xx][yy] = false; break;
						}
					}
				}
			}
			// Victory lap for this x,y square
			for (xx = 0 ; xx < slices.xPart.length-1 ; xx++) {
				for (yy = 0 ; yy < slices.yPart.length-1 ; yy++) {
					if (gridLightedSlices[xx][yy]) {
						total += (slices.xPart[xx+1]-slices.xPart[xx])*(slices.yPart[yy+1]-slices.yPart[yy]);
					}
				}
			}
		}
...
function transformData() {
	var inst, x1, x2, y1, y2, tokens, tokens2, tokens3;
	data = [];
	rawData.forEach(str => {
		tokens = str.split(" ");
		if (tokens.length == 4) {
			...
			tokens3 = tokens[3].split(",");			
		} else {
			inst = (tokens[1] == "on") ? ON : OFF;
			tokens2 = tokens[2].split(",");
			tokens3 = tokens[4].split(",");
		}
		data.push({
			inst : inst, 
			x1 : parseInt(tokens2[0], 10), 
			x2 : parseInt(tokens3[0], 10),
			y1 : parseInt(tokens2[1], 10),
			y2 : parseInt(tokens3[1], 10)});
	});
}
...
function place(p_i) {
	return Math.floor(p_i/50);
}

/*
A 20x20 array that contains the indexes of the grids.
[0][x] : 0-49 in y
[1][x] : 50-49 in y...
[11][4] : the 500-549 in y, 150-199 in x
Note : we can swap x and y, actually
*/
function makeArrayIndexes() {
	var result = generateArrangedDoubleEntryArray(20, 20, function(x, y) {return []});
	for (var i = 0 ; i < rawData.length ; i++) {
		for (x = place(data[i].x1) ; x <= place(data[i].x2) ; x++) {
			for (y = place(data[i].y1) ; y <= place(data[i].y2) ; y++) {
				result[x][y].push(i);
			}
		}
	}
...
// Cuts the (p_xMin-p_xMax, p_yMin-p_yMax) region into rectangular slices separated by the indexes
// If the indexes are [3, 8] and the corresponding minMaxes are (20, 30, 40, 41) and (23, 32, 45, 49), slices are (0, 20, 23, 41, 46, 50) in x and (30, 32, 46, 50) ; y=49 being at the border of the region
function makeSlices(p_indexList, p_xMin, p_xMax, p_yMin, p_yMax) {
	slicesX = [p_xMin, p_xMax+1];
	slicesY = [p_yMin, p_yMax+1];
	var minMax;
	for (var i = 0 ; i < p_indexList.length ; i++) {
		minMax = data[p_indexList[i]];
		if (minMax.x1 > p_xMin) {
			shiftIntoListUniqueNumbers(slicesX, minMax.x1);
		}
		if (minMax.x2 < p_xMax) {
			shiftIntoListUniqueNumbers(slicesX, minMax.x2+1);
		}
		if (minMax.y1 > p_yMin) {
			shiftIntoListUniqueNumbers(slicesY, minMax.y1);
		}
		if (minMax.y2 < p_yMax) {
			shiftIntoListUniqueNumbers(slicesY, minMax.y2+1);
		}
	}
	return {xPart : slicesX, yPart : slicesY};
}




// Copy-pasted on part 1 but with gridLightedSlices + victory lap changed
function conclusion_6_2() {
	...
	for (x = 0 ; x < 20 ; x++) {
		for (y = 0 ; y < 20 ; y++) {
			...
			slX = slices.xPart.length-1;
			slY = slices.yPart.length-1;
			gridLightedSlices = generateDoubleEntryArray(slY, slX, 0); // Don't forget, it's dim2length, dim1length
			for (i = 0 ; i < indexes[x][y].length ; i++) {
				x1Find = 0;
				x2Find = 0;
				y1Find = 0;
				...
				for (xx = x1Find ; xx <= x2Find ; xx++) {
					for (yy = y1Find ; yy <= y2Find ; yy++) {
						switch(data[indexes[x][y][i]].inst) {
							case TOGGLE : gridLightedSlices[xx][yy] +=2; break;
							case ON : gridLightedSlices[xx][yy]++; break;
							case OFF : if (gridLightedSlices[xx][yy] > 0) {gridLightedSlices[xx][yy]--}; break;
						}
					}
				}
			}
			...
			for (xx = 0 ; xx < slices.xPart.length-1 ; xx++) {
				for (yy = 0 ; yy < slices.yPart.length-1 ; yy++) {
					total += gridLightedSlices[xx][yy]*(slices.xPart[xx+1]-slices.xPart[xx])*(slices.yPart[yy+1]-slices.yPart[yy]);
				}
			}
		}
	}
