...
function conclusion_17_1() {
	...
	var dX = [0, -1, 0, 1, 0];
	var dY = [0, 0, -1, 0, 1];
	for (var y = 1 ; y < gYL-1 ; y++) {
		for (var x = 1 ; x < gXL-1 ; x++) {
			for (i = 0 ; i <= 4 ; i++) {				
				if (gData[y+dY[i]][x+dX[i]] != '#') {
					break;
				}
			}
			if (i == 5) {
...
var gXL, gYL, gDistances, gDirectionsRelative, gDirectionsAbsolute;
function conclusion_17_2() {
	...
	for (var y = 0 ; y < gYL ; y++) {
		for (var x = 0 ; x < gXL ; x++) {
			switch (gData[y][x]) {
				case '^' : dirS = DIRECTION.UP ; break;
				case '>' : dirS = DIRECTION.RIGHT ; break;
				case 'v' : dirS = DIRECTION.DOWN ; break;
				case '<' : dirS = DIRECTION.LEFT ; break;
			}
			if (dirS != -1) {
				xS = x;
				yS = y;
	...
	// Find the main path 
	findPath(xS, yS);
	//... and transform it in relative
	gDirectionsRelative = [];
	var dir = dirS;
	var newDir;
	for (var i = 0 ; i < gDirectionsAbsolute.length ; i++) {
		if (TurningLeftDirection[dir] == gDirectionsAbsolute[i]) {
			gDirectionsRelative.push("L");
		} else {
			gDirectionsRelative.push("R");
		}
		dir = gDirectionsAbsolute[i];
	}
	// Now that the patterns have been (humanly) determined :
	var mainRoutine = "A,B,A,C,C,A,B,C,B,B";
	var patternA = "L,8,R,10,L,8,R,8";
	var patternB = "L,12,R,8,R,8";
	var patternC = "L,8,R,6,R,6,R,10,L,8";
	// Run the robot a second time !
	var reader = new newIntCodeReader(rawData17);
	alterIntcodeProgram(reader, 0, 2);
	var outputs;
	provideInputAscii(reader, mainRoutine);
	provideInputAscii(reader, patternA);
	provideInputAscii(reader, patternB);
	provideInputAscii(reader, patternC);
	provideInputAscii(reader, "n");
	outputs = runAndOutputIntcodeProgram(reader);
	// I'll provide all inputs immediately, so it's done :)
	return outputs[outputs.length-1]; // So we needed the last value.
}

function findPath(p_xS, p_yS) {
	gDistances = [];
	gDirectionsAbsolute = [];
	var x = p_xS;
	var y = p_yS;
	var dir;
	// Find the good direction, advance until you can't, repeat.
	existingNeighborsCoorsDirections(x, y, gXL, gYL).forEach(coorsDir => {
		if (gData[coorsDir.y][coorsDir.x] == '#') {
			dir = coorsDir.direction;
			gDirectionsAbsolute.push(dir);
			gDistances.push(0);
		}
	});
	while (neighborExists(x, y, gXL, gYL, dir) && gData[y+DeltaY[dir]][x+DeltaX[dir]] == '#') {
		x += DeltaX[dir];
		y += DeltaY[dir];
		gDistances[gDistances.length-1]++;
	}
	...
	while (!stop) {
		newDir = -1;
		existingNeighborsCoorsDirections(x, y, gXL, gYL).forEach(coorsDir => {
			if (gData[coorsDir.y][coorsDir.x] == '#' && coorsDir.direction != OppositeDirection[dir]) {
				newDir = coorsDir.direction;
				gDirectionsAbsolute.push(newDir);
				gDistances.push(0);
			}
		});
		dir = newDir; // ... if not for a forEach I could have made a break !
		if (newDir == -1) {
			stop = true;
		} else {			
			while (neighborExists(x, y, gXL, gYL, dir) && gData[y+DeltaY[dir]][x+DeltaX[dir]] == '#') {
