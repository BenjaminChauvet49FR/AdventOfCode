...
const DirectionToInput15 = [INPUT15.WEST, INPUT15.NORTH, INPUT15.EAST, INPUT15.SOUTH];

function newEmptySpace(p_from) {
	return TurningRightDirection[p_from];
}
...
function conclusion_15_aux(p_isPart2) {
	var reader = new newIntCodeReader(rawInput15);
	const xStart = 0;
	const yStart = 0;
	const ea = new ExpandableArray(50, 50, 1, 1, 0, 0, -1); // I thought 100x100 grids of size 100x100 would be required :)
	var outputs;
	var x = 0;
	var y = 0;
	ea.put(x, y, DIRECTION.UP); 
	var direction; // Direction to go
	var found = false;
	var foundP2 = false;
	var xGoal, yGoal;
	// Digging the maze : when entering a room (regardless on whether we explore it for the first time or not), make sure all three next directions explored in this room will be (leftward, upward, rightward)
	while (!found) {
		direction = ea.get(x, y); 
		if (foundP2 && x == 0 && y == 0 && direction == DIRECTION.UP) { // Back to start after having found the goal
			found = true;
		}
		provideInput(reader, DirectionToInput15[direction]);
		outputs = runAndOutputIntcodeProgram(reader);
		if (outputs[0] == OUTPUT15.EMPTY || outputs[0] == OUTPUT15.GOAL) {
			x += DeltaX[direction];
			y += DeltaY[direction];
			ea.put(x, y, TurningLeftDirection[direction]); // So we will visit direction (turning left, turning up, turning right, turning down) in that order. Always !
			if (outputs[0] == OUTPUT15.GOAL) {
				if (!p_isPart2) {					
					found = true;
				} else {
					foundP2 = true;
		...
		} else if (outputs[0]== OUTPUT15.WALL) {
			ea.put(x+DeltaX[direction], y+DeltaY[direction], -2)
			ea.put(x, y, TurningRightDirection[direction]);
		}
	}
	// Log !
	var strToLog; 
	for (var y = ea.yMin ; y <= ea.yMax ; y++) {
		strToLog = "";
		for (var x = ea.xMin ; x <= ea.xMax ; x++) {
			if (x == 0 && y == 0) {
				strToLog += "S";
				...
				switch (ea.get(x, y)) {
					case -2 : strToLog += "#"; break;
					case -1 : strToLog += "?"; break;
					default : strToLog += "."; break;
				}
			}
		}
		console.log(strToLog);
	}
	//Find the shortest way (treat -1 tiles as walls and everything else as floor : the maze has no loops, right ?)
	// Replenish all directed spaces at 0 :)
	var distance = 0;
	var spacesToAnalyze = [{x : p_isPart2 ? xGoal : 0, y : p_isPart2 ? yGoal : 0}];
	var newSpacesToAnalyze = [];
	var found = false;
	ea.forAllSpacesComplete(function(x, y) {
		if (ea.get(x, y) > 0) {
			ea.put(x, y, 0);
		}
	});
