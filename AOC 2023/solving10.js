...
function conclusion_10_1() {
	const xys = findStartingPosition();
	// I checked : the loop starts left and up.  So... start left !
	var x = xys.x-1;
	var y = xys.y;
	var dirProv = DIRECTION.RIGHT;
	var length = 1;
	while ((x != xys.x || y != xys.y) && length < 19600) {		
		coors = findNewPosition(x, y, dirProv);
		x = coors.x;
		y = coors.y;
		dirProv = coors.newProvenance;
		length++;
		//console.log(x+","+y+" : " + rawData[y].charAt(x));
	}
...
function findNewPosition(p_x, p_y, p_prov) {
	var x, y, newProv;
	switch(p_prov) {
		case DIRECTION.RIGHT :
			switch (rawData[p_y].charAt(p_x)) {
				case 'L' :
					x = p_x; y = p_y-1; newProv = DIRECTION.DOWN; break;
				case '-' :
					x = p_x-1; y = p_y; newProv = DIRECTION.RIGHT; break;
				case 'F' :
					x = p_x; y = p_y+1; newProv = DIRECTION.UP; break;
			}
		break;
		case DIRECTION.UP :
			switch (rawData[p_y].charAt(p_x)) {
				case 'L' :
					x = p_x+1; y = p_y; newProv = DIRECTION.LEFT; break;
				case '|' :
					x = p_x; y = p_y+1; newProv = DIRECTION.UP; break;
				case 'J' :
					x = p_x-1; y = p_y; newProv = DIRECTION.RIGHT; break;
...
function conclusion_10_2() {
	const xys = findStartingPosition();
	var loopSections = generateDoubleEntryArray(rawData.length, rawData[0].length, false);
	var x = xys.x-1;
	var y = xys.y;
	var dirProv = DIRECTION.RIGHT;
	var length = 1;
	loopSections[y][x] = true;
	while ((x != xys.x || y != xys.y) && length < 19600) {		
		coors = findNewPosition(x, y, dirProv);
		x = coors.x;
		y = coors.y;
	...
	for (var y = 0 ; y < rawData.length ; y++) {
		...
		for (var x = 0 ; x < rawData.length ; x++) {
			if (loopSections[y][x]) {
				...
				if (c == '7') {
					inside = (lastCurve == 'F' ? inside : !inside);
				}
				if (c == 'J') {
					inside = (lastCurve == 'L' ? inside : !inside);
				}				
			} else {
				if (inside) {
					answer++;
