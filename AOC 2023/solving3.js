...
function conclusion_3_1() {
	...
	var str;
	var c, d, x1, x2;
	var currentNumber, foundSymbol;
	var applicable, notUpMost, notDownMost;
	for (var y = 0 ; y < rawData.length ; y++) {
		...
		for (var x = 0 ; x < rawData[0].length ; x++) {
			...
			if (d != UTILS_CHAR_INVALID) {
				...
				x2 = x-1;
				answer += testAdjacency(x1, x2, true, y) ? number : 0;
				number = 0;
				x1 = -1;
			}
		}
		answer += testAdjacency(x1, x2, false, y) ? number : 0; // Change row
	}
	return answer;
}

function testAdjacency(p_x1, p_x2, p_notRightMost, p_y) {
	const notUpMost = (p_y > 0);
	const notDownMost = (p_y < rawData.length-1);
	if (p_x1 > 0) {
		if (isSpecialChar(rawData[p_y].charAt(p_x1-1)) || (notUpMost && isSpecialChar(rawData[p_y-1].charAt(p_x1-1))) || (notDownMost && isSpecialChar(rawData[p_y+1].charAt(p_x1-1))) ) {
			return true;
		}
	}
	if (p_notRightMost) {
		if (isSpecialChar(rawData[p_y].charAt(p_x2+1)) || (notUpMost && isSpecialChar(rawData[p_y-1].charAt(p_x2+1))) || (notDownMost && isSpecialChar(rawData[p_y+1].charAt(p_x2+1))) ) {
			return true;
		}
	}
	for (var x = p_x1 ; x <= p_x2 ; x++) {
		if ( (notUpMost && isSpecialChar(rawData[p_y-1].charAt(x))) || (notDownMost && isSpecialChar(rawData[p_y+1].charAt(x))) ) {
			return true;
		}
	}
	return false;	
...
function conclusion_3_2() {
	...
	var answer = 0;
	var numbers;
	var notLeftMost, notRightMost;
	for (y = 0 ; y < rawData.length ; y++) {
		for (x = 0 ; x < rawData[0].length ; x++) {
			numbers = [];
			if (getChar(x, y) == '*') {
				notLeftMost = (x > 0);
				notRightMost = (x < rawData[0].length-1);
				if (y > 0) {						
					if (digit(getChar(x, y-1)) != UTILS_CHAR_INVALID) {
						numbers.push(readNumberIncluding(x, y-1));
					} else {
						if (notLeftMost && digit(getChar(x-1, y-1)) != UTILS_CHAR_INVALID) 
							numbers.push(readNumberIncluding(x-1, y-1));
						if (notRightMost && digit(getChar(x+1, y-1)) != UTILS_CHAR_INVALID) 
							numbers.push(readNumberIncluding(x+1, y-1));
					}
				} 
				...
				if (notLeftMost && digit(getChar(x-1, y)) != UTILS_CHAR_INVALID) 
					numbers.push(readNumberIncluding(x-1, y));
				if (notRightMost && digit(getChar(x+1, y)) != UTILS_CHAR_INVALID) 
					numbers.push(readNumberIncluding(x+1, y));
				if (numbers.length == 2) {
...
function readNumberIncluding(p_x, p_y) {
	var x = p_x;
	while (x > 0 && digit(getChar(x-1, p_y)) != UTILS_CHAR_INVALID) {
		x--;
	}
	var answer = 0;	
	while (x != rawData[0].length && digit(getChar(x, p_y)) != UTILS_CHAR_INVALID) {
		answer *= 10;
		answer += digit(getChar(x, p_y)); // We see twice !
		x++;
	}
	return answer;
}