...
function isCompatible(p_str) {
	var tokens, tokens2, number;
	tokens = p_str.split(":")[1].split(/,|;/);
	for (var i = 0 ; i < tokens.length ; i++) {
		// Credits : https://stackoverflow.com/questions/650022/how-do-i-split-a-string-with-multiple-separators-in-javascript
		tokens2 = tokens[i].trim().split(" ");
		number = parseInt(tokens2[0], 10);
		switch(tokens2[1]) {
			case "red": 
				if (number > 12) {
					return false;
...
function conclusion_2_2() {
	var answer = 0;
	for (var i = 0 ; i < rawData.length ; i++) {
		answer += minimumNeeded(rawData[i]);
	}
	return answer;
}