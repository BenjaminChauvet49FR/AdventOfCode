...
function init() {
	...
	for (var i = 0 ; i < rawData.length ; i++) {
		...
		ind1 = getIndexInListWithAdd(labels, tokens[0]);
		if (ind1 == gData.length) {
			gData.push([]);
		}
		for (var j = 1 ; j < tokens.length ; j++) {
			ind2 = getIndexInListWithAdd(labels, tokens[j]);
			if (ind2 == gData.length) {
				gData.push([]);
			}
			gData[ind1].push(ind2);
			gData[ind2].push(ind1);
			if (ind1 < ind2) {
				gConnectionsList.push([ind1, ind2]);
			} else {
				gConnectionsList.push([ind2, ind1]);
			}
		}
	}
	gL = gData.length;
...
function conclusion_25_1() {
	init();
	gBelongToGroup = new CheckCollectionGeneric(gL, 0);
	gReached = new CheckCollection(gL);
	var cut = [];
	var answerCut = null;
	var i = 0;
	var iC1, iC2;
	
	// Sort cuts "from best to worst"
	for (var i = 0 ; i < gConnectionsList.length ; i++) {
		gConnectionsList[i].alternateDistance = distanceWithBrokenCut(gConnectionsList[i][0], gConnectionsList[i][1]);
	}
	sortDescending(gConnectionsList, [function(e) {return e.alternateDistance}]);
	
	for (var i = 0 ; i < gConnectionsList.length ; i++) {
		cut.push(gConnectionsList[i]);
		for (var j = 0 ; j < i ; j++) {
			cut.push(gConnectionsList[j]);
				for (var k = 0 ; k < j ; k++) {
					cut.push(gConnectionsList[k]);
					if (areSeparatedNow(cut[2][0], cut[2][1],cut)) {
						answerCut = cut.slice();
					}
					cut.pop();
				}
	...
	var answer2 = 0;
	for (var i = 0 ; i < gL ; i++) {
		if (gBelongToGroup.getGeneric(i) == 1) {
			answer1++;
		} else {
			answer2++;
		}
...
// I assume we must cut three "unique" connections. 
function areSeparatedNow(p_i, p_j, p_cut) {
	gBelongToGroup.clean();
	gBelongToGroup.addGeneric(p_i, 1);
	gBelongToGroup.addGeneric(p_j, 2);
	verticeWithNeighborsToExplore = [p_i, p_j];
	var vertice;
	var i;
	while (verticeWithNeighborsToExplore.length > 0) {
		vertice = verticeWithNeighborsToExplore.pop();
		for (var i = 0 ; i < gData[vertice].length ; i++) {
			ver2 = gData[vertice][i];
			if (areJoined(vertice, ver2, p_cut)) {
				if (gBelongToGroup.getGeneric(ver2) == 0) {
					gBelongToGroup.addGeneric(ver2, gBelongToGroup.getGeneric(vertice));
					verticeWithNeighborsToExplore.push(ver2);
				} else if (gBelongToGroup.getGeneric(ver2) != gBelongToGroup.getGeneric(vertice)) {
					return false;
				}
			}
		}
...
function areJoined(p_x, p_y, p_cut) {
	for (var i = 0 ; i < p_cut.length ; i++) {
		if (p_cut[i][0] == p_x && p_cut[i][1] == p_y) {
			return false;
		}
		if (p_cut[i][0] == p_y && p_cut[i][1] == p_x) {
			return false;
		}
	}
	return true;
...
// What is the closest from index 1 and ic2 if we aren't allowed to cut the link between them ? 
function distanceWithBrokenCut(p_ic1, p_ic2) {
	gReached.clean();
	gReached.add(p_ic1);
	var nodesToExplore = [p_ic1];
	var futureNodesToExplore;
	var toEx, oth;
	var distance = 1;
	while (nodesToExplore.length > 0) {
		futureNodesToExplore = [];
		for (var j = 0 ; j < nodesToExplore.length ; j++) {			
			toEx = nodesToExplore[j];
			for (var i = 0 ; i < gData[toEx].length ; i++) {
				oth = gData[toEx][i];
				if ((oth != p_ic2 || toEx != p_ic1) && gReached.add(oth)) {
					if (oth == p_ic2) {
						return distance;
					}
					futureNodesToExplore.push(oth);
				}
			}
			nodesToExplore = futureNodesToExplore.slice();
			distance++;
		}
	}
