...
function initData() {
	...
	var c, iS, iF; // index siblings, index father
	for (var i = 0 ; i < rawData.length ; i++) {
		c = rawData.charAt(i);
		// NEWS : extend last path
		// ( : create new path. Its father is the previous path.
		// | : create new path. Its father is the father of the previous path.
		// ) : set the next path for this path and for its siblings (actually done in '(' and '|'. Set the children for the common father to these. Create new path.
		switch (c) {
			case 'N' : gData.paths[gData.paths.length-1] += "N"; break;
			case 'E' : gData.paths[gData.paths.length-1] += "E"; break;
			case 'W' : gData.paths[gData.paths.length-1] += "W"; break;
			case 'S' : gData.paths[gData.paths.length-1] += "S"; break;
			case '(' : 
				gData.paths.push("");
				gData.pathChildren.push([]);
				iF = gData.paths.length-2;
				gData.pathFathers.push(iF);
				gData.pathChildren[iF].push(gData.pathFathers.length-1);
				break;
			case '|' : 
				gData.paths.push("");
				gData.pathChildren.push([]);
				iF = gData.pathFathers[gData.pathFathers.length-1];
				gData.pathFathers.push(iF);
				gData.pathChildren[iF].push(gData.pathFathers.length-1);
				break;
			case ')' :
				iS = gData.pathFathers.length-1;
				iF = gData.pathFathers[iS];
				while (gData.pathFathers[iS] == iF) {
					gData.pathChildren[iS].push(gData.paths.length); // Index of the future string
					iS--;
				}
				gData.paths.push("");
				gData.pathFathers.push(gData.pathFathers[iF]); 
				gData.pathChildren.push([]);
				break;
			default : break;
		}
...
function runPaths(p_indexStart, p_deltaXStart, p_deltaYStart) {
	var dx = p_deltaXStart;
	var dy = p_deltaYStart;
	// Run NEWS
		...
		switch(gData.paths[p_indexStart].charAt(i)) {
			case 'E' :
				...
				/*if (dx+gData.xStart == gData.xLength-1) {
					gData.xLength += 10;
					for (var y = 0 ; y < global.yLength ; y++) {
						for (var x = 0 ; x < 10 ; x++) {
							gData.rooms[y].push({wallR : CLOSED, wallD : CLOSED});
						}
					}
				}*/ // I will go straight
			...
			case 'S' : 
				...
				/*if (dy+gData.yStart == gData.yLength-1) {
					gData.yLength += 10;
					for (var y = 0 ; y < 10 ; y++) {
						gData.rooms.push(generateArrangedArray(gData.xLength,
							function(xx, yy) {return {wallR : CLOSED, wallD : CLOSED} }
						);
					}
				}*/
			...
			case 'W' : 
				...
				/*if (dx+gData.xStart == gData.xLength-2) {
					gData.xLength += 10;
					gData.xStart -= 10;
					for (var y = 0 ; y < global.yLength ; y++) { // Expand, then shift to the right
						for (var x = 0 ; x < 10 ; x++) {
							gData.rooms[y].push({wallR : CLOSED, wallD : CLOSED});
						}
						for (var x = gData.xLength-10 ; x < gData.xLength-10 ; x++) {
							shiftedRoom = gData.rooms[y][x];
							gData.rooms[y][x+10] = {wallR : shiftedRoom.wallR, wallD : shiftedRoom.wallD};
						}
						for (var x = 0 ; x < 10 ; x++) {
							gData.rooms[y][x] = ({wallR : CLOSED, wallD : CLOSED});
						}
					}
				}*/ // TODO : NOT TESTED
	...
	// Special case because there are many dead-ends with an empty string as an alternative (not nice - I observed the string) :
	if (dy == p_deltaYStart && dx == p_deltaXStart && gData.paths[p_indexStart].length > 0)  {
		return;
	}
	
	// Run all children
	gData.pathChildren[p_indexStart].forEach(iChild => {
		runPaths(iChild, dx, dy);
	});
}
...
function consoleLogPaths() {
	...
	for (var y = 0 ; y < gData.rooms2.length ; y++) {
		...
		for (var x = 0 ; x < gData.rooms2[y].length ; x++) {
			...
			if (wR == OPEN && wD == CLOSED) {
				string1 += "..";
				string2 += "XX";
			}
			if (wR == CLOSED && wD == OPEN) {
				string1 += ".X";
				string2 += ".X";
			}
			if (wR == OPEN && wD == OPEN) {
