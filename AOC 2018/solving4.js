...
var dataEvents, dataSleeps, dataSleepsGuardIDs;
...
function setup() {	
	...
	var tokens;
	id = -1;
	// Gather all dataEvents
	for (var i = 0 ; i < rawData.length ; i++) {
		date = dateInYear(parseInt(rawData[i].substring(6, 8),10), parseInt(rawData[i].substring(9, 11),10) );
		if (rawData[i].substring(12, 14) != "00") { // Starts minute at midnight the day after.
			minute = 0;
			date++;
		} else {			
			minute = parseInt(rawData[i].substring(15, 17));
		}
		switch (rawData[i].charAt(19)) {
			case 'f':
				action = ACTION.SLEEP; 
				break;
			case 'w':
			...
			case 'G':
				...
				id = parseInt(tokens[3].substring(1));
				break;
		}
		dataEvents.push({
			date : date,
	...
	// Sort dataEvents by chronological order
	...
	// Reaffect IDs correctly
	var currentID = 0;
	for (var i = 0 ; i < dataEvents.length ; i++) {
		if (dataEvents[i].action == ACTION.BEGIN) {
			currentID = dataEvents[i].id;
		} else {
			dataEvents[i].id = currentID;
		}
	}
	// Re-sort, through IDs this time.
	dataEvents.sort(function(a,b) {
		if (a.id != b.id) {
			return a.id-b.id;
		}
	...
	// Generates intervals for each elf (doesn't take date into account though) :
	dataSleeps = [];
	dataSleepsGuardIDs = [];
	var id = -1;
	for (var i = 0 ; i < dataEvents.length ; i++) {
		switch (dataEvents[i].action) {
			case ACTION.BEGIN:
				if (i == 0 || dataEvents[i].id != dataEvents[i-1].id) {
					dataSleeps.push([]);
					dataSleepsGuardIDs.push(dataEvents[i].id);
				}
			break;
			case ACTION.SLEEP:
				// If next action is a different day/guard, set the interval to XX-59. Otherwise, set the interval to XX-(date-1).
				// Assumption : no two consecutive sleep events !
				if (i == dataEvents.length-1 || (dataEvents[i+1].id != dataEvents[i].id) || (dataEvents[i+1].day != dataEvents[i].day)) {
					dataSleeps[dataSleepsGuardIDs.length-1].push([dataEvents[i].minute, 59]);
				} else {
					dataSleeps[dataSleepsGuardIDs.length-1].push([dataEvents[i].minute, dataEvents[i+1].minute]);
				}
			break;
		}
	}	
...
function conclusion_4_1() {
	// Lists all events (begin, sleep, wake up) then sorts them by id of guard and then chronologically 
	setup();
	// For each guard, sum their interval sleep times
	var total = 0;
	var sleepMax = 0;
	var indexMax = 0; // Not guard id !
	for (var i = 0 ; i < dataSleeps.length ; i++) {
		total = 0;
		for (var j = 0 ; j < dataSleeps[i].length ; j++) {
			total += dataSleeps[i][j][1];
			total -= dataSleeps[i][j][0];
		}
		if (sleepMax < total) {
			sleepMax = total;
			indexMax = i;
	...
	// Now, the guard who sleeps most is the one at position indexMax
	arrayMinutes = generateArray(60, 0);
	for (var j = 0 ; j < dataSleeps[indexMax].length ; j++) {
		for (var k = dataSleeps[indexMax][j][0] ; k <= dataSleeps[indexMax][j][1] ; k++) {
			arrayMinutes[k]++;
		}
	}
	var sleepyMinute = 0;
	for (var k = 0 ; k <= 59 ; k++) {
		if (arrayMinutes[k] > arrayMinutes[sleepyMinute]) {
			sleepyMinute = k;
		}
	}
	return sleepyMinute * dataSleepsGuardIDs[indexMax];
}
...
function conclusion_4_2() {
	...
	for (var i = 0 ; i < dataSleeps.length ; i++) {			
		arrayMinutes = generateArray(60, 0);
		for (var j = 0 ; j < dataSleeps[i].length ; j++) {
			for (var k = dataSleeps[i][j][0] ; k <= dataSleeps[i][j][1] ; k++) {
				arrayMinutes[k]++;
			}
		}
		for (var k = 0 ; k <= 59 ; k++) {
			if (arrayMinutes[k] > bestTotal) {
				bestMinute = k;
				bestIndex = i;
				bestTotal = arrayMinutes[k];
			}
		}
	}
	return bestMinute * dataSleepsGuardIDs[bestIndex];
}