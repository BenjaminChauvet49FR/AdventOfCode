...
function conclusion_3_1() {
	...
	for (var i = 0 ; i < data.length ; i++) {
		array1 = sortString(data[i].slice(0, data[i].length/2));
...
// Returns one common character between 2 array characters... already sorted, right ?
function commonChar(p_array1, p_array2) {
	var i1 = 0;
	var i2 = 0;
	while (i1 < p_array1.length && i2 < p_array2.length) {
		if (p_array1[i1] < p_array2[i2]) {
			i1++;
		} else if (p_array1[i1] > p_array2[i2]) {
			i2++;
		} else {
			return p_array1[i1];
		}
...
function conclusion_3_2() {
	...
	for (var i = 0 ; i < data.length ; i+=3) {
		array1 = sortString(data[i]);
		array2 = sortString(data[i+1]);
		array3 = sortString(data[i+2]);
...
function commonChars(p_sortedArray1, p_sortedArray2) {
	var answer = [];
	var i1 = 0;
	var i2 = 0;
	while (i1 < p_sortedArray1.length && i2 < p_sortedArray2.length) {
		if (p_sortedArray1[i1] < p_sortedArray2[i2]) {
			i1++;
		} else if (p_sortedArray1[i1] > p_sortedArray2[i2]) {
			i2++;
		} else {
			answer.push(p_sortedArray1[i1]);
			i1++;
			i2++;
		}
	}
